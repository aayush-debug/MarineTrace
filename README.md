...
[**Explore Architecture**](docs/ARCHITECTURE.md) • [**API Contract**](docs/API_CONTRACT.md) • [**Project Structure**](PROJECT_STRUCTURE.md) • [**Docker Guide**](docs/DOCKER_GUIDE.md) • [**Cloud Deployment**](docs/DEPLOYMENT_GUIDE.md)
...
### 1. 🛰️ SpaceShift™ Real-Time Global Surveillance Suite
- **Global World Overview & Target Matrix**: Interactive world map overview with live indicator markers across key strategic chokepoints (Mumbai Offshore/Bombay High, Gulf of Kutch, Palk Strait, Andaman Sea, Strait of Malacca, Persian Gulf, English Channel).
- **Synchronized Radar Telemetry Feed**: Live data ingestion with automatic resilience fallback (`/spcsft/live-feed`, `/spcsft/health`) ensuring continuous operational readiness in both connected and air-gapped environments.
- **Direct Target-to-Investigation Bridge**: Instant single-click transition from satellite detection dot directly into full forensic investigation workspace.

### 2. 🛰️ Sentinel-1 SAR Oil Slick Detection (Computer Vision)
...
### 3. 🌊 Hydrodynamic Advection & Drift Physics (OpenDrift)
...
### 4. 🚢 AIS Traffic Reconstruction & 3-Stage Filtering
...
### 5. ⚖️ 5-Factor Explainable Attribution Model
...
### 6. 📑 Official Incident Intelligence Dossier (GoI / ICG Standard)
- **Official Office Memorandum (OM) Format**: Formatted according to Government of India / Indian Coast Guard statutory standards with official classification stamps (`RESTRICTED // LAW ENFORCEMENT SENSITIVE`), executive determinations, and investigator sign-off blocks.
- **PDF Print Isolation**: Fully automated `@media print` rules that suppress navigation headers and UI chrome to produce clean A4 documents.

### 7. 💻 Professional Maritime Workstation UI
- **Defense-Grade Cartography**: Leaflet GIS map with locked global bounds (`minZoom=3`, `noWrap=true`), custom tactical HUD, and NATO/IMO symbology.
- **Dynamic Light & Dark Theme Engine**: Instant reactive switching between Dark Tactical Workstation and High-Contrast Enterprise Light modes with local persistence.
- **Operator Access & Audit Security**: Single-persona Coast Guard Operator switch (`Cmdr. Vikram Malhotra`) with immutable activity logging (`/logs`).
...
### 2. 🚀 All-in-One Startup (Recommended)
Run the entire platform concurrently with automatic Docker detection:
```bash
npm start
# or
bash start.sh
```
*If Docker is active, it starts containerized services; otherwise, it automatically starts native FastAPI and Vite development servers.*
...
### 4. ⚙️ Running Manually (Native Processes)
...
### 5. 🧪 Standalone CLI Demo (Zero Setup)
Execute the complete end-to-end investigation pipeline directly in terminal:
```bash
python run_demo.py
```
...
### 6. 🌐 Localhost Access Endpoints & Web Portals
...
| **📋 Audit Access Logs** | [`http://localhost:5173`](http://localhost:5173) (via `/logs`) | Tamper-evident operator access and case investigation trail |
...
| **Frontend Application** | TypeScript + Vite | 2,460 modules | ✅ **0 ERRORS** |
...
│   ├── src/api/          # Modular API client endpoints (spcsft, investigation, etc.)
...
│   ├── src/context/      # Investigation, SpaceShift & Theme state stores
...
├── docs/                 # 📚 Architecture, Docker, API, and Deployment specifications
│   ├── ARCHITECTURE.md
│   ├── API_CONTRACT.md
...
├── docker-start.sh       # Docker startup automation script
├── start.sh              # Universal auto-detecting startup script
├── vercel.json           # Vercel cloud deployment SPA routing config
...
| `GET` | `/spcsft/live-feed` | Live synchronized SAR surveillance targets and telemetry |
| `GET` | `/spcsft/health` | SpaceShift radar and telemetry subsystem status |
