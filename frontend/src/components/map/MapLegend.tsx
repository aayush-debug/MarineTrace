...
export const MapLegend: React.FC = () => {
  ...
  return (
    <div className="absolute bottom-3 left-3 z-[1000] bg-white/95 text-slate-900 border-slate-300 dark:bg-[#111622]/95 dark:text-slate-200 dark:border-[#1e293b] rounded-lg p-2.5 shadow-2xl text-xs select-none pointer-events-auto max-w-xs backdrop-blur-md font-sans border">
      <div
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center justify-between gap-2 text-slate-900 dark:text-slate-200 font-bold cursor-pointer text-[11px] uppercase tracking-wider font-mono"
      >
        <div className="flex items-center gap-1.5">
          <Layers className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400" />
          <span>Cartographic Symbology</span>
        </div>
        <button className="text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 cursor-pointer">
          {isOpen ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronUp className="w-3.5 h-3.5" />}
        </button>
      </div>

      {isOpen && (
        <div className="space-y-1.5 mt-2 pt-2 border-t border-slate-200 dark:border-slate-800 text-[11px]">
          {legendItems.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 text-slate-800 dark:text-slate-300 font-medium">
              {item.shape === 'rect' && (
                <span
