...
export const TopNav: React.FC = () => {
  ...
  return (
    <>
      <header className="h-12 bg-[#111622] border-b border-[#1e293b] px-3 sm:px-4 flex items-center justify-between gap-2 select-none z-30 sticky top-0 shrink-0 shadow-sm no-print">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0 shrink-0">
          ...
          >
            ...
            <div className="flex items-baseline gap-1.5 shrink-0">
              <span className="font-semibold text-slate-100 text-sm tracking-tight whitespace-nowrap font-mono">
                MarineTrace
              </span>
          ...
          {investigation ? (
            <div className="hidden sm:flex items-center gap-1 shrink-0">
              ...
              <button
                ...
                title="Open Target Oil Spill Selector"
              >
                Switch Spill
              </button>
            </div>
            ...
            <button
              onClick={() => setIsIncidentSelectorOpen(true)}
              className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/40 text-blue-300 text-xs font-medium cursor-pointer transition-colors shrink-0"
            >
        ...
        {/* CENTER: Operational Classification (Only on ultra-wide screens to prevent pushing right controls) */}
        <div className="hidden 2xl:flex items-center gap-2.5 px-3 py-1 bg-[#0c1017] rounded border border-slate-800/80 text-xs font-mono text-slate-400 shrink-0 whitespace-nowrap">
          <span className="flex items-center gap-1.5 text-emerald-400 text-[10px] font-semibold shrink-0">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span>C2 LINK ONLINE</span>
          </span>
          <span className="text-slate-700">|</span>
          <span className="text-[10px] text-slate-300">SENTINEL-1A SAR</span>
          <span className="text-slate-700">|</span>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-auto whitespace-nowrap">
          ...
          <div
            ...
            className="hidden md:flex items-center gap-2 font-mono text-xs px-2 py-1 bg-[#0c1017] border border-[#1e293b] rounded text-slate-300 select-none shrink-0 whitespace-nowrap cursor-pointer hover:border-slate-700 transition-colors"
          ...
          <button
            onClick={executeDemo}
            disabled={loading}
            className="hidden lg:flex items-center gap-1.5 px-2 py-1 bg-[#161e2e] hover:bg-[#1c2638] border border-[#1e293b] text-slate-200 hover:text-white rounded text-xs font-medium transition-colors disabled:opacity-50 shrink-0 whitespace-nowrap cursor-pointer"
          >
          ...
          <div className="relative pl-1 border-l border-slate-800 shrink-0" ref={menuRef}>
            {isAuthenticated && user ? (
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold transition-colors cursor-pointer shrink-0 shadow"
              >
      ...
      {/* ── OPERATOR LOG IN & ROLE SELECTION MODAL ── */}
      {isLoginModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#111622] border border-[#1e293b] rounded-xl shadow-2xl max-w-md w-full overflow-hidden text-slate-200 animate-in fade-in zoom-in-95 font-sans">
            {/* Modal Header */}
            <div className="p-4 bg-[#161e2e] border-b border-[#1e293b] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-100 text-sm font-mono">
                    MarineTrace Command Authentication
                  </h3>
                  <p className="text-[11px] text-slate-400 font-mono">
                    Select Authorized Maritime Agency Role
                  </p>
                </div>
              </div>
              <button
              ...
              <div className="space-y-2">
                {[
                  ...
                  <button
                    key={item.roleKey}
                    onClick={async () => {
                      await loginAsDemo(item.roleKey);
                      setIsLoginModalOpen(false);
                    }}
                    className="w-full p-3 rounded-lg bg-[#161e2e] hover:bg-[#1c2638] border border-[#1e293b] hover:border-blue-500/60 text-left transition-all cursor-pointer group"
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-100 text-xs group-hover:text-blue-300 transition-colors">
                        {item.name}
                      </span>
                    ...
                    <div className="text-[11px] text-blue-400 font-mono mt-0.5">{item.role}</div>
            ...
            {/* Modal Footer */}
            <div className="p-3 bg-[#0c1017] border-t border-[#1e293b] text-center text-[11px] font-mono text-slate-500">
              MARPOL 73/78 Annex I Compliance System · Encrypted Session
            </div>
          </div>
        </div>
      )}
