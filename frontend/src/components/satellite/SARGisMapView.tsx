...
export const SARGisMapView: React.FC<SARGisMapViewProps> = ({
  ...
  const selectedCoords = selectedCandidate?.geo_coordinates as [number, number][] | undefined;

  const activeBasemap = BASEMAP_CONFIGS[basemap] || BASEMAP_CONFIGS['google-hybrid'];

  // Suspect Vessels at Time of SAR Acquisition
  ...
  return (
    <div className="relative w-full h-full bg-[#030712] overflow-hidden">
      ...
      >
        ...
        {/* Layer 4: Candidate Spill Geometries */}
        {showCandidateContours &&
          scene.candidates.map((cand) => {
            ...
            // Generate polygon lat/lons from candidate geo_coordinates
            const coords: [number, number][] =
              (cand.geo_coordinates as [number, number][]) || [
                [cand.centroid.latitude - 0.015, cand.centroid.longitude - 0.025],
                [cand.centroid.latitude + 0.02, cand.centroid.longitude - 0.01],
                [cand.centroid.latitude + 0.015, cand.centroid.longitude + 0.03],
                [cand.centroid.latitude - 0.02, cand.centroid.longitude + 0.015],
              ];
            ...
            return (
              <React.Fragment key={cand.candidate_id}>
                ...
                {showCandidateLabels && (
                  <CircleMarker
                    center={[cand.centroid.latitude, cand.centroid.longitude]}
      ...
      <div className="absolute top-3 right-3 z-10 flex flex-col gap-2 max-w-xs">
        ...
        <div className="bg-slate-950/90 backdrop-blur-md border border-slate-800/80 rounded-lg p-3 shadow-xl text-xs font-mono space-y-3">
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
            <span className="font-bold text-slate-200 flex items-center gap-1.5">
              <Layers className="w-3.5 h-3.5 text-cyan-400" />
              <span>Google Maps GIS View</span>
            </span>
            <span className="text-[10px] text-cyan-400 font-semibold px-1.5 py-0.5 rounded bg-cyan-950/60 border border-cyan-800/40">
              Live Satellite
            </span>
          </div>
          ...
          <div className="pt-2 border-t border-slate-800/80 space-y-1.5">
            <div className="flex items-center justify-between text-[10px]">
              ...
              <span className="text-cyan-300 font-bold">{(sarOverlayOpacity * 100).toFixed(0)}%</span>
            </div>
            <input
              type="range"
              min="0"
      ...
      <div className="absolute bottom-3 left-3 z-10 flex items-center gap-3 px-3 py-1.5 bg-slate-950/90 backdrop-blur-md border border-slate-800/80 rounded-lg shadow-lg font-mono text-[10px] text-slate-300">
